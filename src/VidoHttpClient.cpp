...
#include "VidoHttpClient.h"
#include "b64.h"

// Initialize constants
const char* VidoHttpClient::kUserAgent = "Arduino/2.2.0";
const char* VidoHttpClient::kContentLengthPrefix = HTTP_HEADER_CONTENT_LENGTH ": ";
const char* VidoHttpClient::kTransferEncodingChunked = HTTP_HEADER_TRANSFER_ENCODING ": " HTTP_HEADER_VALUE_CHUNKED;

VidoHttpClient::VidoHttpClient(VidoGsmClientSim800& aClient, const char* aServerName, uint16_t aServerPort)
...
VidoHttpClient::VidoHttpClient(VidoGsmClientSim800& aClient, const String& aServerName, uint16_t aServerPort)
 : VidoHttpClient(aClient, aServerName.c_str(), aServerPort)
{
}

VidoHttpClient::VidoHttpClient(VidoGsmClientSim800& aClient, const IPAddress& aServerAddress, uint16_t aServerPort)
...
void VidoHttpClient::resetState()
{
  iState = eIdle;
  iStatusCode = 0;
...
void VidoHttpClient::stop()
{
  iClient->stopSafe();
  resetState();
}

void VidoHttpClient::connectionKeepAlive()
{
  iConnectionClose = false;
}

void VidoHttpClient::noDefaultRequestHeaders()
{
  iSendDefaultRequestHeaders = false;
}

void VidoHttpClient::beginRequest()
{
  iState = eRequestStarted;
}

int VidoHttpClient::startRequest(const char* aURLPath, const char* aHttpMethod, 
...
int VidoHttpClient::sendInitialHeaders(const char* aURLPath, const char* aHttpMethod)
{
#ifdef LOGGING
    ...
    String message = "";
    message = String(aHttpMethod) + " " + aURLPath + " HTTP/1.1\r\n";    
    iClient->write(message.c_str());
    ...
    if (iSendDefaultRequestHeaders)
    {
        ...
        if (iServerName)
        {
            message = "Host: " + String(iServerName);
            if (iServerPort != kHttpPort)
            {
                message += ":" + String(iServerPort);
            }
            message += "\r\n";
...
void VidoHttpClient::sendHeader(const char* aHeader)
{
    String message = String(aHeader) + "\r\n"; 
    iClient->write(message.c_str());
}

void VidoHttpClient::sendHeader(const char* aHeaderName, const char* aHeaderValue)
{
    String message = String(aHeaderName) + ": " + String(aHeaderValue) + "\r\n";
    iClient->write(message.c_str());
}

void VidoHttpClient::sendHeader(const char* aHeaderName, const int aHeaderValue)
{
    String message = String(aHeaderName) + ": " + String(aHeaderValue) + "\r\n";
    iClient->write(message.c_str());
}

void VidoHttpClient::sendBasicAuth(const char* aUser, const char* aPassword)
{
    // Send the initial part of this header line
    iClient->write("Authorization: Basic ");
...
void VidoHttpClient::finishHeaders()
{
    iClient->write("\r\n");
    iState = eRequestSent;
}

void VidoHttpClient::flushClientRx()
{
    while (iClient->availableSafe())
    {
        iClient->read();
    }
}

void VidoHttpClient::endRequest()
{
    beginBody();
}

void VidoHttpClient::beginBody()
{
...
int VidoHttpClient::get(const char* aURLPath)
{
    return startRequest(aURLPath, HTTP_METHOD_GET);
}

int VidoHttpClient::get(const String& aURLPath)
{
    return get(aURLPath.c_str());
}

int VidoHttpClient::post(const char* aURLPath)
{
    return startRequest(aURLPath, HTTP_METHOD_POST);
}

int VidoHttpClient::post(const String& aURLPath)
{
    return post(aURLPath.c_str());
}

int VidoHttpClient::post(const char* aURLPath, const char* aContentType, const char* aBody)
{
    return post(aURLPath, aContentType, strlen(aBody), (const byte*)aBody);
}

int VidoHttpClient::post(const String& aURLPath, const String& aContentType, const String& aBody)
{
    return post(aURLPath.c_str(), aContentType.c_str(), aBody.length(), (const byte*)aBody.c_str());
}

int VidoHttpClient::post(const char* aURLPath, const char* aContentType, int aContentLength, const byte aBody[])
{
    return startRequest(aURLPath, HTTP_METHOD_POST, aContentType, aContentLength, aBody);
}

int VidoHttpClient::put(const char* aURLPath)
{
    return startRequest(aURLPath, HTTP_METHOD_PUT);
}

int VidoHttpClient::put(const String& aURLPath)
{
    return put(aURLPath.c_str());
}

int VidoHttpClient::put(const char* aURLPath, const char* aContentType, const char* aBody)
{
    return put(aURLPath, aContentType, strlen(aBody),  (const byte*)aBody);
}

int VidoHttpClient::put(const String& aURLPath, const String& aContentType, const String& aBody)
{
    return put(aURLPath.c_str(), aContentType.c_str(), aBody.length(), (const byte*)aBody.c_str());
}

int VidoHttpClient::put(const char* aURLPath, const char* aContentType, int aContentLength, const byte aBody[])
{
    return startRequest(aURLPath, HTTP_METHOD_PUT, aContentType, aContentLength, aBody);
}

int VidoHttpClient::patch(const char* aURLPath)
{
    return startRequest(aURLPath, HTTP_METHOD_PATCH);
}

int VidoHttpClient::patch(const String& aURLPath)
{
    return patch(aURLPath.c_str());
}

int VidoHttpClient::patch(const char* aURLPath, const char* aContentType, const char* aBody)
{
    return patch(aURLPath, aContentType, strlen(aBody),  (const byte*)aBody);
}

int VidoHttpClient::patch(const String& aURLPath, const String& aContentType, const String& aBody)
{
    return patch(aURLPath.c_str(), aContentType.c_str(), aBody.length(), (const byte*)aBody.c_str());
}

int VidoHttpClient::patch(const char* aURLPath, const char* aContentType, int aContentLength, const byte aBody[])
{
    return startRequest(aURLPath, HTTP_METHOD_PATCH, aContentType, aContentLength, aBody);
}

int VidoHttpClient::del(const char* aURLPath)
{
    return startRequest(aURLPath, HTTP_METHOD_DELETE);
}

int VidoHttpClient::del(const String& aURLPath)
{
    return del(aURLPath.c_str());
}

int VidoHttpClient::del(const char* aURLPath, const char* aContentType, const char* aBody)
{
    return del(aURLPath, aContentType, strlen(aBody),  (const byte*)aBody);
}

int VidoHttpClient::del(const String& aURLPath, const String& aContentType, const String& aBody)
{
    return del(aURLPath.c_str(), aContentType.c_str(), aBody.length(), (const byte*)aBody.c_str());
}

int VidoHttpClient::del(const char* aURLPath, const char* aContentType, int aContentLength, const byte aBody[])
{
    return startRequest(aURLPath, HTTP_METHOD_DELETE, aContentType, aContentLength, aBody);
}

int VidoHttpClient::responseStatusCode()
{
    if (iState < eRequestSent)
    {
        return HTTP_ERROR_API;
...
int VidoHttpClient::skipResponseHeaders()
{
...
String VidoHttpClient::responseBody()
{
...
bool VidoHttpClient::endOfBodyReached()
{
...
int VidoHttpClient::available()
{
    ...
    int clientAvailable = iClient->availableSafe();
...
bool VidoHttpClient::headerAvailable()
{
...
String VidoHttpClient::readHeaderName()
{
...
String VidoHttpClient::readHeaderValue()
{
    int colonIndex = iHeaderLine.indexOf(':');
...
int VidoHttpClient::read(uint8_t *buf, size_t size)
{
